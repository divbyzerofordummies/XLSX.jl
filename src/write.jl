...
function update_worksheets_xml!(xl::XLSXFile)
    ...
    for i in 1:sheetcount(wb)
        ...
        # Since we do not at the moment track changes, we need to delete all data and re-write it.
        # Column formatting is preserved in the <cols> subtree.
        # Row formatting would get lost if we simply remove the <sheetData> structure storing the rows
        ...
        let
            ...
            for c in child_nodes # the <row> tags
        ...
        # TODO: Is it reasonable to have different spans if less columns exist in a row?
